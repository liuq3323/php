...
class DfpayController extends Controller
{
    ...
    public function add()
    {
        ...
        $subbranch = I("post.subbranch", '', 'string,strip_tags,htmlspecialchars');
        if(!$subbranch) {
        ...
        $province = I("post.province", '', 'string,strip_tags,htmlspecialchars');
        if(!$province) {
            $this->showmessage('省份不能为空！');
        }
        $city = I("post.city", '', 'string,strip_tags,htmlspecialchars');
        if(!$city) {
