...
class UserController extends BaseController
{
    ...
    public function exportuser()
    {
        ...
        $title = array('用户名', '商户号', '用户类型', '上级用户名', '状态', '认证', '可用余额', '冻结余额', '投诉保证金', '注册时间');
        $data  = M('Member')
            ->where($map)
            ->select();
        foreach ($data as $item) {
            ...
            $deposit  = ComplaintsDepositModel::getComplaintsDeposit($item['id']);
            $list[] = array(
                ...
                'complaintsDeposit' => number_format((double) $deposit['complaintsDeposit'], 2, '.', ''),
