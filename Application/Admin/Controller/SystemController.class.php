        ...
        if (IS_POST) {
            ...
            if ($verifyGoogle && $auth_type == 1) {//谷歌安全码验证
                ...
                if(!$code) {
                    ...
                    if (session('send.sysconfig') != $code || !$this->checkSessionTime('sysconfig', $code)) {
...
EOD;
    ...
    public function clearData() {
        ...
        if(IS_POST) {
            ...
            foreach($type_ids as $v) {
                if($v == 1) { //入金记录
                    $res = M('order')->where(['pay_applydate'=>['between', [$startTime, $endTime]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                } elseif($v == 2) {//出金记录
                    $res = M('wttklist')->where(['sqdatetime'=>['between', [date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                    $res = M('tklist')->where(['sqdatetime'=>['between', [date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                    $res = M('df_api_order')->where(['create_time'=>['between', [$startTime, $endTime]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                } elseif($v == 3) {//登录记录
                    $res = M('loginrecord')->where(['logindatetime'=>['between', [$startTime, $endTime]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                } elseif($v == 4) {//冻结记录
                    $res = M('blockedlog')->where(['createtime'=>['between', [date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                    $res = M('complaints_deposit')->where(['create_at'=>['between', [$startTime, $endTime]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
                } elseif($v == 5) {//资金记录
                    $res = M('moneychange')->where(['datetime'=>['between', [date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();
                    if(FALSE === $res) {
                        M()->rollback();
                        $this->ajaxReturn(['status' => 0, 'msg' => "清理失败！"]);
                    }
    ...
    public function sysconfigSend()
    {
        $uid               = session('admin_auth')['uid'];
        $user = M('Admin')->where(['id'=>$uid])->find();
        $res = $this->send('sysconfig', $user['mobile'] ,'系统设置');
        $this->ajaxReturn(['status' => $res['code']]);
    }
