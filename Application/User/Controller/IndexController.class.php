...
FROM pay_order WHERE pay_applydate >= UNIX_TIMESTAMP(  '".$firstday."' ) AND pay_successdate < UNIX_TIMESTAMP(  '".
        ...
        //今日已付订单数
        $stat['todayorderpaidcount'] = M('Order')->where(['pay_memberid'=>10000+$this->fans['uid'],'pay_applydate'=>['between', [strtotime($todayBegin), strtotime($todyEnd)]], 'pay_status'=>['in', '1,2']])->count();
        //今日未付订单数
        ...
        //今日实付金额
        $stat['todayorderactualsum'] = M('Order')->where(['pay_memberid'=>10000+$this->fans['uid'],'pay_applydate'=>['between', [strtotime($todayBegin), strtotime($todyEnd)]], 'pay_status'=>['in', '1,2']])->sum('pay_actualamount');
        //投诉保证金
    ...
    public function showcontent()
    {
        ...
        $record = M('browserecord')->where(['articleid' => $id, 'userid' => $this->fans['uid']])->count();
        if(!$record) {
