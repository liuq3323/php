...
class UnionpayWgPcController extends PayController
{
    ...
    public function Pay($array)
    {
        ...
        $encryp = encryptDecrypt(serialize($return), 'UnionpayWgPc');
    ...
    public function Rpay()
    {
        ...
        $data = I('post.', '');
        //将数据解密并反序列化
        $return = unserialize(encryptDecrypt($data['encryp'], 'UnionpayWgPc', 1));
        //检测数据是否正确
    ...
    public function callbackurl()
    {
        ...
        if($pay_status <> 0){
            $this->EditMoney($_REQUEST["orderid"], 'UnionpayWgPc', 1);
        }else{
            exit("error");
        }
    }
