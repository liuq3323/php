...
class WftWxSmRsaController extends PayController{
	...
	public function Pay($array){
		...
		$notifyurl = $this->_site . 'Pay_WftWxSmRsa_notifyurl.html';

		$callbackurl = $this->_site . 'Pay_WftWxSmRsa_callbackurl.html';
	...
	public function callbackurl(){
        ...
        if($pay_status <> 0){
            $this->EditMoney($_REQUEST["orderid"], 'WftWxSmRsa', 1);
        }else{
            exit("error");
        }
	}
    ...
    public function notifyurl(){
        ...
        if($this->resHandler->isTenpaySign()){
            if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){
                $this->EditMoney($out_trade_no, 'WftWxSmRsa', 0);
                echo 'success';
                exit();
            } else {
                echo 'failure4';
